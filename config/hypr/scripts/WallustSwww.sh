...
wallust_targets=(
  ...
  "$HOME/.config/wallust/p10k-colors.zsh"
  "$HOME/.config/nvim/lua/colors/wallust-colors.lua"
)
...
# Reload nvim colorscheme in all running instances
for sock in $(find "/run/user/$(id -u)" /tmp -maxdepth 5 -name "nvim.*.0" -type s 2>/dev/null); do
  nvim --server "$sock" --remote-send ':lua package.loaded["colors.wallust-colors"]=nil vim.cmd("colorscheme wallust")<CR>' 2>/dev/null &
done
